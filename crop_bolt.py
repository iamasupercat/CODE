...
def yolo_to_bbox(yolo_line, img_width, img_height, img_name=None, line_num=None):
    ...
    # 좌표 값(x_center, y_center, width, height)만 0~1 범위 체크
    # 각도(angle)는 라디안 값이므로 0~1 범위가 아님
    coord_count = min(4, len(vals))  # 최대 4개 좌표 값만 체크
    for i in range(coord_count):
        v = vals[i]
        if not (0.0 <= v <= 1.0):
            print(f"[경고] {img_name or ''} 라벨 {line_num or ''} 좌표 값 {v}가 0~1 범위가 아님: {yolo_line.strip()}")
...
def process_set(set_type):
    ...
    if not os.path.exists(images_dir):
        print(f"경로가 존재하지 않음: {images_dir}")
        return
